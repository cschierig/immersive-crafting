...
public class DayTimeCondition implements ICCondition {
    ...
    public static final class Serializer implements ICConditionSerializer<DayTimeCondition> {
        ...
        public static final MapCodec<DayTimeCondition> CODEC = RecordCodecBuilder.mapCodec(
                instance -> instance.group(
                        Codec.intRange(0, 24000).fieldOf("start_time").forGetter(con -> con.startTime),
                        Codec.intRange(0, 24000).fieldOf("end_time").forGetter(con -> con.endTime)
                ).apply(instance, DayTimeCondition::new)
        );

        public static final StreamCodec<RegistryFriendlyByteBuf, DayTimeCondition> STREAM_CODEC = StreamCodec.composite(
                ByteBufCodecs.INT,
                con -> con.startTime,
                ByteBufCodecs.INT,
                con -> con.endTime,
                DayTimeCondition::new
        );

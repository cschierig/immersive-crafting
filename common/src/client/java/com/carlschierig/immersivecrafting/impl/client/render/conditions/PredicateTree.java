package com.carlschierig.immersivecrafting.impl.client.render.conditions;
...
public class PredicateTree {
    ...
    public PredicateTree(ICPredicate predicate) {
        ...
        try {
            ...
            ICUtil.LOG.error(e.getMessage());
            for (var element : e.getStackTrace()) {
                ICUtil.LOG.error(element.toString());
            }
        }
    }
    ...
    private void renderSubtree(Node node, GuiGraphicsExtractor draw, int x, int y, float delta) {
        ...
        // Draw condition icon
        var renderer = ICClientRegistries.RENDERABLE.getValue(node.condition.getRenderer());
        renderer.renderUnknown(node.condition, draw, x, y, delta);
        draw.pose().popMatrix();

        // Draw tooltip
        setTooltipNode(node, draw, x, y, delta);
    ...
    private void setTooltipNode(Node node, GuiGraphicsExtractor draw, int x, int y, float delta) {
    ...
    public void renderTooltip(GuiGraphicsExtractor draw, int x, int y, float delta) {
        if (tooltipNode != null) {
            var tooltip = tooltipNode.condition.getTooltip();
